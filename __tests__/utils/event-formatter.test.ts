...
describe('formatEventTitle', () => {
  ...
  it('formats home matches with vs and home emoji', () => {
    ...
    expect(formatEventTitle('Team A', 'Match details', undefined, 'Home')).toBe('🏟️ Team A');
  });
  ...
  it('formats away matches with vs and bus emoji', () => {
    ...
    expect(formatEventTitle('Team A', 'Match details', undefined, 'Away')).toBe('✈️ Team A');
  });
  ...
  it('formats cup events with trophy emoji', () => {
    expect(formatEventTitle('Team A', 'Cup Tournament', 'Cup')).toBe('🏆 Team A (Cup Tournament)');
    expect(formatEventTitle('P2015 Blå', 'Stockholm Cup', 'Cup')).toBe(
      '🏆 P2015 Blå (Stockholm Cup)'
    );
    expect(formatEventTitle('F2012', 'Regional Cup Finals', 'Cup')).toBe(
      '🏆 F2012 (Regional Cup Finals)'
    );
    expect(formatEventTitle('Team A', 'Team A Cup Match', 'Cup')).toBe(
      '🏆 Team A (Team A Cup Match)'
    );
  });
});
